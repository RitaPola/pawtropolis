...
public class DropActionStrategy implements ActionStrategy {
    ...
    public void execute() {
        if (bag.getItems().isEmpty()) {
            inputController.getInputString("> There are no items in the bag\n");
            return;
        }
        String itemName = inputController.getInputItemName("> Which item do you want to drop? \n");
        Item item = bag.removeItemByName(itemName);
        if (itemName == null || itemName.equalsIgnoreCase("> no ") || itemName.equals(" ")) {
            inputController.getInputString("> You have not specified an item to drop \n");
            return;
        }
        if (item == null) {
            inputController.getInputString("> Item not found in the bag \n ");
            return;
        }
